    ...
    <ul className="flex justify-between flex-wrap items-center gap-1 mt-auto w-full">
        ...
        <div className="text-primary capitalize inline-flex gap-4 ms-auto">
            ...
            product.category.map((cat, idx) => {
                return (
                <Link
                    href={`/categories/${cat.name.toLowerCase()}/products`}
                    key={idx}
                >
                    {cat.name}
                </Link>
        ...
        <div className=" capitalize inline-flex gap-8 ms-auto text-slate-700">
            <Link
            target="_blank"
            href={`https://x.com/intent/post?url=${typeof window !== 'undefined' ? window.location.origin : ''}${pathname}`}
            >
            <FaXTwitter className="hover:text-primary" />
            </Link>
            <Link
            target="_blank"
                href={`https://www.instagram.com/share?url=${typeof window !== 'undefined' ? window.location.origin : ''}${pathname}`}
            >
            <FaInstagram className="hover:text-primary" />
            </Link>
            <Link
            target="_blank"
            href={`https://facebook.com/sharer/sharer.php?u=${typeof window !== 'undefined' ? window.location.origin : ''}${pathname}`}
            >
